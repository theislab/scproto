...
class PrototypeModel(VariationalAutoencoder):
    def __init__(self, num_prototypes, model, model_loss, **kwds) -> None:
    ...
    def forward(self, x):
        ...
        return z, self.decoder(z), output
...
class PrototypeLoss:
    def __init__(self) -> None:
        self.vae, self.model_loss, self.interpretablity = 0, 0, 0
        self.loss = 0

    def calculate(self, x, x_hat, z, y, y_pred, prot_model: PrototypeModel):
        self.vae = vae_loss(x, x_hat, prot_model.encoder.kl)
        self.model_loss = prot_model.model_loss(y_pred, y)
...
def train_step(model: PrototypeModel, data_loader, optimizer, device):
    model.to(device)
