...
class SwAV(ScpoliTrainer):
    def __init__(self, dataset=None, dump_name_version=3) -> None:
        print('new plot by aug added')
        super().__init__(dataset) 
        self.experiment_name = 'swav_no-cvae'
        
                #########################
        #### data parameters ####
        #########################
        self.nmb_crops = [8]
        self.augmentation_type = 'knn'
        # self.size_crops = [224]
        # self.min_scale_crops = [0.14]
        # self.max_scale_crops = [1]
        ...
        self.base_lr = 4.8
        self.final_lr = 0
        self.freeze_prototypes_niters = 313
        self.wd = 1e-6
        ...
        #########################
        # self.arch = "resnet50"
        # self.hidden_mlp = 2048
        self.workers = 10
        self.checkpoint_freq = 25
        self.use_fp16 = True
        self.sync_bn = "pytorch"
        self.syncbn_process_group_size = 8
        self.dump_name_version = dump_name_version
        ...
        self.device = 'cuda'
        self.debug = False
        self.freezable_prototypes = True
    ...
    def build_optimizer(self):
        ...
        warmup_lr_schedule = np.linspace(self.start_warmup, self.base_lr, len(self.train_loader) * self.warmup_epochs)
        iters = np.arange(len(self.train_loader) * (self.epochs - self.warmup_epochs))
        cosine_lr_schedule = np.array([
            self.final_lr + 0.5 * (self.base_lr - self.final_lr) * (1 + math.cos(math.pi * t / (len(self.train_loader) * (self.epochs - self.warmup_epochs)))) for t in iters
        ])
    ...
    def log_wandb_loss(self, scores):
        ...
        wandb.log({'swav': swav_loss, 'cvae': cvae_loss, 'loss': avg_loss, 'prot loss': prot_loss})
    ...
    def train_one_epoch(self, epoch):
        ...
        for iteration, inputs in enumerate(self.train_loader):
            ...
            loss = swav_loss + cvae_loss * self.cvae_reg + prot_decoding_loss
            ...
            if self.freezable_prototypes:
                if epoch * len(self.train_loader) + iteration < self.freeze_prototypes_niters:
        ...
        return (epoch, losses.avg, cvae_losses.avg, swav_losses.avg, prot_decoding_losses.avg), self.queue 
            ...
            with torch.no_grad():
                ...
                if self.queue is not None:
                    if use_the_queue or not torch.all(self.queue[i, -1, :] == 0):
                        use_the_queue = True
                        queue_output = torch.mm(self.queue[i], self.model.module.prototypes.weight.t())
    ...
    def calculate_umaps(self, trained_model = True):
        ...
        self.cell_types, self.study_labels = self.ref.adata.obs.cell_type, self.ref.adata.obs.study
        self.embedding_umap, self.prototype_umap = calculate_umap(embeddings, prototypes)
        self.outputs_umap, _ = calculate_umap(outputs)
        
    def plot_ref_prot_umap(self):
        plot_umap(self.embedding_umap, self.prototype_umap, self.cell_types, self.study_labels)
    ...
    def plot_by_augmentation(self, n_samples, n_augmentations):
        ...
        plot_umap(cell_umap, prototype_umap, inputs['celltypes'].reshape(n_samples*n_augmentations).cpu(), all_labels)
